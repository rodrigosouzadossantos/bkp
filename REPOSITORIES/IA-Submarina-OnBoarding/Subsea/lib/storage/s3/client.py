...
class S3ClientError( RuntimeError ) :
    '''High-level S3 client error.'''

class S3AuthError( RuntimeError ) :
    '''Authentication or authorization failure with S3.'''
...
class S3Client :
  ...
  def _handle_auth_error( self, e : Exception ) -> None :
    self.log.critical(
      'S3 authentication/authorization failed (bucket=%s): %s',
      self.bucket,
      e,
    )
    raise S3AuthError( str( e ) ) from None

  def _handle_client_error( self, e : ClientError ) -> None :
    code = (
      e.response
        .get( 'Error', { } )
        .get( 'Code', '' )
    ...
    raise S3ClientError(
      f"Failed to access S3 bucket '{self.bucket}': {code}"
    ) from None
