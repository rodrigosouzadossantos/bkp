...
def format_tb(value: float) -> str:
    ...
    return "{} TB ({} TiB)".format(round(value / (1000 ** 5), 2),round(value / (1024 ** 5), 2))
...
def create_cards_row(df: pl.DataFrame) -> widgets.HBox:
    ...
    last_months_storage = (
        df.filter(
            pl.col( 'files_oldest' ) >= pl.lit( datetime.now( ) - relativedelta( months = months ) ),
            pl.col( 'files_oldest' ).is_not_null( )
        ).with_columns(
            pl.col( 'files_oldest' ).dt.date( ).alias( 'day' )
        )
        .group_by( 'day' )
        .agg([
        ...
        .select(
            human_bytes( pl.col( 'files_size' ).mean( ).alias( 'avg' ) ),
            human_bytes_IEC( pl.col( 'files_size' ).mean( ).alias( 'avg_IEC' ) )
        )
    )

    card_storage = create_card("Total Storage", format_tb(total_storage_tb) )
    card_ingestion = create_card(f'Ingestion Rate / Day ({months} months)', "{} ({})".format(last_months_storage['avg'].item(),last_months_storage['avg_IEC'].item() ) )
    return widgets.HBox(
        [
            card_storage,
...
# YEAR SLIDER
# ===========================================================
def create_year_slider(df: pl.DataFrame) -> widgets.IntSlider:
...
# BUILD PLOT
# ===========================================================
def build_figure(years, monthly, slider_year, df) -> (go.Figure, widgets.Output):
    ...
    size_min, size_max, files_min, files_max = compute_global_ranges(df)
    ...
    for yr in years:
        df_year = monthly.filter(pl.col("year") == yr).sort("month")
        size_vals = [None] * 12
        count_vals = [None] * 12
...
# SLIDER CALLBACK
# ===========================================================
def attach_slider_callback(fig, slider_year, years, output):
...
def compute_bucket_global_range(monthly: pl.DataFrame, column = "total_size"):
...
def build_bucket_figure(monthly_bucket, slider_year):
    ...
    buckets = sorted(monthly_bucket["bucket"].unique().to_list())
    # Prepare figure
    fig = go.Figure()
    # Precompute global Y range
    min_val, max_val = compute_bucket_global_range(monthly_bucket)
    for bucket in buckets:
        # Filter bucket for all years
        df_bucket = monthly_bucket.filter(pl.col("bucket") == bucket)
        # For slider: show only selected year
        df_year = df_bucket.filter(pl.col("year") == slider_year.value)
...
def attach_bucket_slider_callback(fig, slider_year, monthly_bucket, output):
    ...
    def on_change(change):
        ...
        df_year = monthly_bucket.filter(pl.col("year") == year)
        # Update traces one bucket at a time
        for i, bucket in enumerate(sorted(monthly_bucket["bucket"].unique().to_list())):
            # padded 12 months
            yvals = [None] * 12
            df_bucket = df_year.filter(pl.col("bucket") == bucket)
...
def build_bucket_cumulative_figure(monthly_bucket, slider_year):
    months_labels = ["Jan","Feb","Mar","Apr","May","Jun",
                     "Jul","Aug","Sep","Oct","Nov","Dec"]
    buckets = sorted(monthly_bucket["bucket"].unique().to_list())
    ...
    for bucket in buckets:
        df_bucket = monthly_bucket.filter(
            (pl.col("bucket") == bucket) & (pl.col("year") == slider_year.value)
        )
...
def attach__bucket_slider_callback(fig, slider, monthly_bucket, output, plot_func):
...
def build_dashboard(df: pl.DataFrame):
    ...
    fig, plot_output = build_figure(years, monthly, slider_year, df)
    attach_slider_callback(fig, slider_year, years, plot_output)
    
    # ----- SECOND PLOT: BUCKET AGGREGATION -----
    monthly_bucket = compute_monthly_by_bucket(df)

    bucket_fig, bucket_output, _ = build_bucket_figure(monthly_bucket, slider_year)
    attach_bucket_slider_callback(bucket_fig, slider_year, monthly_bucket, bucket_output)
    ...
    monthly_bucket_cum = (
        ...
        .group_by(["bucket", "year", "month"])
        .agg([
            pl.col("files_size").sum().alias("total_size"),
            pl.col("files_count").sum().alias("total_files")
        ])
        .sort(["bucket", "year", "month"])
        .with_columns([
            pl.col("total_size").cum_sum().over("bucket").alias("total_size_cum"),
            pl.col("total_files").cum_sum().over("bucket")
        ])
    )
    growth_fig, growth_output, _ = build_bucket_cumulative_figure(monthly_bucket_cum, slider_year)
    attach__bucket_slider_callback(growth_fig, slider_year, monthly_bucket_cum, growth_output, build_bucket_cumulative_figure)
    ...
    # tab_class = make_tab("Charts", "<p>More content here.</p>")
    tab_class = build_service_class_tab(df)
    
    tab_sankey = build_sankey_tab( df )

    tabs = widgets.Tab(children=[overview, tab_class, tab_sankey])

    tabs.set_title(0, "Overview")
    tabs.set_title(1, "Class")
    tabs.set_title(2, "Sankey")
