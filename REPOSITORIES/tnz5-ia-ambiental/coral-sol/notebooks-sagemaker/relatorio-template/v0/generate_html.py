...
if 'Scores do modelo' in df.columns:
    def max_score(x):
        if pd.isna(x) or str(x).strip().lower() == 'nan' or str(x).strip() == '':
            return ''
        vals = [float(v) for v in str(x).replace('[','').replace(']','').split(',') if v.strip() != '']
        if not vals:
            return ''
        return f"{max(vals)*100:.1f}%"
    df['Scores do modelo'] = df['Scores do modelo'].apply(max_score)
