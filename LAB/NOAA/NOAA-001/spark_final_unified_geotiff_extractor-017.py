...
ICEBERG_SCHEMA = StructType([
    ...
    StructField("sidecars",
        MapType(StringType(), BooleanType()), True),

    StructField("overviews",
...
def extract_ifd(path):
    ...
    with tifffile.TiffFile(path) as tif:
        ...
        planar = safe_tag(tags, "PlanarConfiguration")
        ...
        if isinstance(sample_format, tuple):
            ...
            sample_format = ", ".join([
                SAMPLE_FORMAT_MAP.get(
                    getattr(x, "value", x),
                    str(x)
                )
                for x in sample_format
            ])

        else:

            sample_format = SAMPLE_FORMAT_MAP.get(
                getattr(sample_format, "value", sample_format),
                str(sample_format)
            )

        bits = safe_tag(tags, "BitsPerSample")
        ...
        return {
            ...
            "compression":
                map_enum(compression, COMPRESSION_MAP),

            "photometric":
                map_enum(photometric, PHOTOMETRIC_MAP),

            "planar_config":
                map_enum(planar, PLANAR_MAP),

            "rows_per_strip":
                safe_tag(tags, "RowsPerStrip"),

            "tile_width":
                safe_tag(tags, "TileWidth"),

            "tile_length":
                safe_tag(tags, "TileLength"),

            "predictor":
                safe_tag(tags, "Predictor"),

            "bits_per_sample":
                bits,

            "samples_per_pixel":
                safe_tag(tags, "SamplesPerPixel"),

            "sample_format":
                sample_format,

            "orientation":
                map_enum(orientation, ORIENTATION_MAP),
        }
    ...
    with tifffile.TiffFile(path) as tif:
        ...
        try:
            ...
            for k, v in gt.items():
                ...
                if k == "ProjectionGeoKey":
                    continue

                if isinstance(v, (list, tuple)):
                    geo_keys[str(k)] = str(list(v))
                else:
        ...
        return {
            ...
            "model_pixel_scale":
                safe_array(tags, "ModelPixelScaleTag"),

            "model_tiepoint":
                safe_array(tags, "ModelTiepointTag"),
...
def detect_sidecars(path):

    return {

        "aux_xml_exists":
            os.path.exists(path + ".aux.xml"),

        "ovr_exists":
            os.path.exists(path + ".ovr"),
...
def extract_tiff(path):
    ...
    return {
        ...
        "raster": {
            ...
            "crs_wkt":
                srs.ExportToWkt() if srs else None,
        ...
        "quality": {
            "band_stats":
                extract_band_stats(ds)
        },

        "sidecars":
            detect_sidecars(path),

        "overviews":
            extract_overviews(ds),
